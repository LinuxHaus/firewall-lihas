...
if ($fw_privclients) {
  ...
  my %ifaces;
	...
	foreach my $iface (keys(%{$ifaces{"physical"}})) {
		...
		if ( $iface =~ /^lo$/ ) {
      print $FILEfilter "nft add rule inet filter OUTPUT counter fwd-$logicaliface\n";
      print $FILEnat "nft add rule inet nat OUTPUT counter pre-$logicaliface\n";
      print $FILEnat "nft add rule inet nat POSTROUTING oifname $iface counter post-$logicaliface\n";
      print $FILEfilter "nft add rule inet filter OUTPUT counter dns-fwd-$logicaliface\n";
      print $FILEnat "nft add rule inet nat OUTPUT counter dns-pre-$logicaliface\n";
      print $FILEnat "nft add rule inet nat POSTROUTING oifname $iface counter dns-post-$logicaliface\n";
		} else {
	    ...
	    print $FILEnat "nft add rule inet nat PREROUTING iifname $iface counter pre-$logicaliface\n";
	    print $FILEnat "nft add rule inet nat POSTROUTING oifname $iface counter post-$logicaliface\n";
	    print $FILEnat "nft add rule inet nat PREROUTING iifname $iface counter dns-pre-$logicaliface\n";
	    print $FILEnat "nft add rule inet nat POSTROUTING oifname $iface counter dns-post-$logicaliface\n";
		}
	}
